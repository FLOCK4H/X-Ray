...
def iprint(string):
    print(f'{ColorCodes.BRIGHT}{ColorCodes.AQUA}[{ColorCodes.ORANGE}X-Ray{ColorCodes.AQUA}]{ColorCodes.RESET}' + ' ' + f'{ColorCodes.WHITE}{ColorCodes.BRIGHT}{string}{ColorCodes.BRIGHT}{ColorCodes.RESET} ')
...
def cinput(string, color=ColorCodes.CYAN, b=False, maxlen=None):
    try:
        ...
        while True:
            ...
            # Handle Enter manually if user presses it before maxlen
            if char in ('\r', '\n'):
                break
...
def rinput(color=ColorCodes.CYAN):
    try:
        r = input(f'{ColorCodes.BRIGHT}{ColorCodes.PURPLE}[{ColorCodes.AQUA}X-Ray{ColorCodes.PURPLE}]{ColorCodes.AQUA} > {ColorCodes.RESET}')
        return r
    except EOFError:
        print("\n")
        exit(0)